...
void my_ldi(champion_t *champion, inst_t *instruction)
{
    ...
    compute_instruction_arg_value(champion, instruction, FIRST_ARG, OP_IND_SIZE);
    compute_instruction_arg_value(champion, instruction, SECONDE_ARG, OP_IND_SIZE);
    s = instruction->arg[FIRST_ARG] + instruction->arg[SECONDE_ARG];

    champion->reg[instruction->arg[THIRD_ARG] - 1] = get_int_from_core(
        pc_arithm(champion->pc_ptr, s % IDX_MOD), OP_REG_SIZE);
